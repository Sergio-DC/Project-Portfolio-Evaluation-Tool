define(['jquery'], function($) {
    ...
    methods.displayPeriodos = function(numero_periodos){//Despliega filas de periodos con 3 columnas 
        var template;
        if(numero_periodos == "")
            template = "";
        else{
    ...
    methods.displayCumCashFlow = function(numero_periodos, cumCashFlow){
        for(var i = 1; i <= numero_periodos; i++){
            $(`#cumCash${i}`).val(cumCashFlow[i]);
        }
    }

    methods.getOutflows = function(numero_periodos){
        var data = [];
    ...
    methods.getInflows = function(numero_periodos){
        var data = [];
    ...
    methods.calculateNetCashFlow = function(numero_periodos, inflows, outflows){//Calcula el flujo neto de caja de los N periodos
        var netCash = [];
    ...
    methods.calculateNPV = function(interes,numero_periodos){//Calcula el Net Present Value, recibe 2 argumentos: interés y periodos
        var npv = [];
    ...
    methods.calculateDiscCashFlow = function(netCashFlow, npv, numero_periodos){
        var discCashFlow = [];
        for(var i = 1; i <= numero_periodos; i++)
    ...
    methods.calculateCumCashFlow = function(principal, discCashFlow, periodos){
